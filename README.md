...
#### 3) 开发者需要维护 用户帐号ID-->MIMC帐号ID 映射吗？
```
不需要，MIMC用户登录/消息收发等都使用APP自己帐号系统里的ID，MIMC帐号体系对APP开发者透明
举例说明：
    假设开发者拥有某APP叫"阅听小说"，集成了MIMC(小米即时消息云)服务
    假设"阅听小说"再小米开放平台注册的appId为"580012345678"
    某用户A登录"阅听小说"所用帐号名为手机号"13800000001"
    某用户B登录"阅听小说"所用帐号名为手机号"13800000002"

    当用户A首次登录"阅听小说"时，MIMC后台服务会为A创建MIMC ID("8049600000000000")，并维护以下映射：
        "580012345678" + "13800000001" --> "8049601705934840"
    然后，用户A给用户B发消息: sendMessage("13800000002", "Hello B");
        由于B还未登录过"阅听小说"，所以MIMC会自动为B创建创建MIMC ID("8049600000000001")
	并将消息"Hello B"暂存在服务器(七天后过期)
    用户B登录"阅听小说"时，因为B在MIMC已经有了ID，所以MIMC后台服务不会再为其分配新ID
    MIMC后台服务检测到B登录，下发离线消息"Hello B"给B
```

#### 4) 既然MIMC帐号体系对APP开发者透明，那么APP如何接入MIMC帐号认证？
###### 请参阅：
* [如何接入](#如何接入)
* [安全认证](#安全认证)

#### 5) MIMC是否支持图片/语音/消息撤回/消息已读/红包等高级功能？
```
