...
#### 回调发送与失败重试
```
回调服务将App用户的即时消息和离线消息以JSON格式POST发给应用方，回调服务收到Web返回的200状态码则表示接收成功。当消息回调失败（返回状态码非200、返回超时、发送失败等）时，系统会一段时间后重试发送最多3次（5s后，30s后，5min后）。未来会上线App应用方的后台管理系统，应用方可以通过管理系统更新URL。
```
* [联系我们](#联系我们)
