...
###### 举例说明：
```
    假设开发者拥有某APP叫"言士文学小说"，集成了MIMC(小米即时消息云)服务
    假设"言士文学小说"在小米开放平台注册的appId为"580012345678"
    某用户A登录"言士文学小说"所用帐号名为手机号"13800000001"
    某用户B登录"言士文学小说"所用帐号名为手机号"13800000002"
 ...
        由于B还未登录过"言士文学小说"，所以MIMC会自动为B创建创建MIMC ID("8049600000000002")
	并维护以下映射：
            "580012345678" + "13800000002" --> "8049600000000002"
	并将消息"Hello B"暂存在服务器(七天后过期)
	
    用户B登录"言士文学小说"时，因为B在MIMC已经有了ID，所以MIMC后台服务不会再为其分配新ID
