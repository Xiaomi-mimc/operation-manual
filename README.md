...
## 消息回调
即时消息云提供消息回调功能，应用方提供Web URL，云服务可以将App用户的即时消息、离线消息以JSON格式Post发给应用方(URL的提供参考 联系我们，建议加QQ群)。使用消息回调功能可以帮助应用方完全掌控App使用情况，回调消息可用于数据挖掘、统计、监控、App保活等。系统收到Web返回的200状态码则表示接收成功。当消息回调失败（返回状态码非200、返回超时、发送失败等）时，系统会重试发送最多3次（5s后，30s后，5min后）。未来会上线App应用方的后台管理系统，应用方可以通过管理系统更新URL。
* [联系我们](#联系我们)
